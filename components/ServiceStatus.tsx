    ...
    if (!selectedProperty) {
        return (
            <div className="flex flex-col items-center justify-center py-20 text-center animate-in fade-in duration-500">
                ...
                <h3 className="text-2xl font-black text-gray-900 tracking-tight">Select a Location</h3>
                <p className="text-gray-500 max-w-xs mt-2 font-medium">Choose a specific address from the property selector to view live collection status.</p>
            </div>
        );
    }
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    {nextPickup?.status === 'in-progress' ? (
                        ...
                        <Card className="bg-gray-900 text-white border-none flex flex-col md:flex-row items-center justify-between gap-8 p-10 shadow-2xl">
                            ...
                            <div className="text-center md:text-right border-t md:border-t-0 md:border-l border-white/10 pt-8 md:pt-0 md:pl-10">
                                <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Service Level</p>
                                <div className="flex items-center gap-2 mt-2 justify-center md:justify-end">
                                    <div className="w-2 h-2 rounded-full bg-green-500" />
                                    <span className="text-lg font-black uppercase tracking-tight">Standard</span>
                                </div>
                            </div>
                        </Card>
                    )}
                ...
                <div className="space-y-6">
                    <h2 className="text-xs font-black text-gray-400 uppercase tracking-widest">Collection History</h2>
                    <Card className="border-none ring-1 ring-base-200 h-full p-0 overflow-hidden">
                        <div className="p-6 border-b border-base-100 bg-gray-50/50">
                            <h4 className="font-black text-gray-900 uppercase text-[10px] tracking-widest">Recent Visits</h4>
                        </div>
                        <div className="divide-y divide-base-100">
                            {[
                                { date: 'July 18', event: 'Waste Collected', status: 'success' },
                                { date: 'July 11', event: 'Waste Collected', status: 'success' },
                                { date: 'July 04', event: 'Holiday Skip', status: 'info' },
                            ].map((log, i) => (
                                <div key={i} className="p-5 flex justify-between items-center hover:bg-gray-50 transition-colors">
                                    <div>
                                        ...
                                        <p className="text-[10px] text-gray-400 font-bold uppercase mt-1">{log.date}</p>
                                    </div>
                                    {log.status === 'success' ? (
