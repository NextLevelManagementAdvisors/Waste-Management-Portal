...
vi.mock('../storage', () => ({
  storage: {
    ...
    createW9: vi.fn(),
    getOpenRoutes: vi.fn(),
    getDriverCustomZones: vi.fn(),
    getRoutesInDriverCoverage: vi.fn(),
...
beforeEach(() => {
  ...
  vi.mocked((storage as any).getDriverCustomZones).mockResolvedValue([]);
  vi.mocked((storage as any).getRoutesInDriverCoverage).mockResolvedValue([]);
...
describe('GET /api/team/routes', () => {
  ...
  it('returns 200 with routes list', async () => {
    vi.mocked((storage as any).getDriverCustomZones).mockResolvedValueOnce([{ id: 'z1', status: 'active' }]);
    vi.mocked((storage as any).getRoutesInDriverCoverage).mockResolvedValueOnce([mockRoute]);
    vi.mocked(storage.getRouteStops).mockResolvedValueOnce([] as any);
    const res = await supertest(createDriverAuthApp()).get('/api/team/routes');
    expect(res.status).toBe(200);
