...
import { generateCollectionDates } from '../optimoSyncService';
import { detectCollectionDayFromHistory } from '../collectionDayDetector';
...
describe('generateCollectionDates', () => {
  ...
  it('defaults to weekly if unknown frequency is passed', () => {
    const dates = generateCollectionDates('thursday', 'unknown-freq', 28);
...
describe('detectCollectionDayFromHistory', () => {
  it('returns null when fewer than 3 completed pickups', () => {
    ...
    expect(detectCollectionDayFromHistory(history)).toBeNull();
  });
  ...
  it('handles empty history array', () => {
    expect(detectCollectionDayFromHistory([])).toBeNull();
  });
