...
export function formatRouteForClient(row: any) {
  ...
  return {
    ...
    estimatedOrders: row.estimated_orders ?? row.estimatedOrders,
    ...
    orderCount: row.order_count != null ? Number(row.order_count) : (row.orderCount != null ? Number(row.orderCount) : undefined),
    completedOrderCount: row.completed_order_count != null ? Number(row.completed_order_count) : (row.completedOrderCount != null ? Number(row.completedOrderCount) : undefined),
