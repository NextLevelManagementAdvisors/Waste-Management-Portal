...
CREATE TABLE IF NOT EXISTS providers (
  ...
  onboarding_status VARCHAR(50) DEFAULT 'pending_territory' NOT NULL,
  stripe_account_id VARCHAR(255),
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
ALTER TABLE providers ADD COLUMN IF NOT EXISTS status VARCHAR(50) DEFAULT 'active' NOT NULL;
ALTER TABLE providers ADD COLUMN IF NOT EXISTS onboarding_status VARCHAR(50) DEFAULT 'pending_territory' NOT NULL;
ALTER TABLE providers ADD COLUMN IF NOT EXISTS stripe_account_id VARCHAR(255);
...
ALTER TABLE provider_territories ADD COLUMN IF NOT EXISTS zone_type VARCHAR(20) NOT NULL DEFAULT 'polygon';
ALTER TABLE provider_territories ADD COLUMN IF NOT EXISTS polygon_coords JSONB;
ALTER TABLE provider_territories ADD COLUMN IF NOT EXISTS zip_codes TEXT[];
ALTER TABLE provider_territories ADD COLUMN IF NOT EXISTS color VARCHAR(7) DEFAULT '#3B82F6';
ALTER TABLE provider_territories ADD COLUMN IF NOT EXISTS status VARCHAR(20) NOT NULL DEFAULT 'active';
ALTER TABLE provider_territories ADD COLUMN IF NOT EXISTS default_pickup_day VARCHAR(10);
CREATE INDEX IF NOT EXISTS idx_provider_territories_provider ON provider_territories(provider_id);
CREATE INDEX IF NOT EXISTS idx_provider_territories_status ON provider_territories(status);

ALTER TABLE locations ADD COLUMN IF NOT EXISTS provider_id UUID REFERENCES providers(id) ON DELETE SET NULL;
ALTER TABLE locations ADD COLUMN IF NOT EXISTS provider_territory_id UUID REFERENCES provider_territories(id) ON DELETE SET NULL;
CREATE INDEX IF NOT EXISTS idx_locations_provider ON locations(provider_id);
CREATE INDEX IF NOT EXISTS idx_locations_provider_territory ON locations(provider_territory_id);
...
CREATE INDEX IF NOT EXISTS idx_swap_recommendations_status ON swap_recommendations(status);
CREATE INDEX IF NOT EXISTS idx_swap_recommendations_provider_a ON swap_recommendations(provider_a_id);
CREATE INDEX IF NOT EXISTS idx_swap_recommendations_provider_b ON swap_recommendations(provider_b_id);
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_swap_recommendations_status') THEN
    ALTER TABLE swap_recommendations ADD CONSTRAINT chk_swap_recommendations_status
